...
int find_boost_unordered_set(boost::unordered_set<int64_t>& s, vector<int64_t>& result)
{
  int count = 0;
  for(vector<int64_t>::iterator it = result.begin(); it != result.end(); ++it)
  {
...
int main(int argc, char *argv[])
{
  ...
  boost::unordered_set<int64_t> boostset(temp.begin(), temp.end());
  ...
  for(int i = 0; i < 2; i++)
  {
    ...
    gettimeofday(&start, NULL);
    cout << "boost set result:" << find_boost_unordered_set(boostset, result) << std::endl;
    gettimeofday(&end, NULL);
    us = (end.tv_sec - start.tv_sec)*1000000 + (end.tv_usec - start.tv_usec);
    cout << "set take time: " << us << std::endl << std::endl;
